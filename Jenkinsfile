node {
  ...
  ansibleVault action: 'decrypt', content: '', input: 'test-file', installation: 'ansible 2.2.1.0', output: '', vaultCredentialsId: 'vault-pass'
...
//    ansibleVault action: 'decrypt /var/jenkins_home/workspace/scm_pipeline/test-file', content: '', input: '', output: '', vaultCredentialsId: 'vault-pass'
    ...
    /*
    stage('unvault') {
         ansibleVault([vaultCredentialsId: 'vaultpass', action: 'decrypt' ] 
         )
    }
    */
